...
export default function Restore() {
  ...
  const getDeleteBoardList = async () => {
    try {
      ...
      console.log("삭제한 게시글 목록 조회 중 에러 발생 : " + error)
    }
  }

  // 삭제한 게시글 복구 기동기 요청 함수
  const restoreBoard = async (board) => {
    if( window.confirm(board.boardTitle + "글을 복구 시키겠습니까?")) {
  ...
  /**
   * useEffect : React의 함수형 컴포넌트에서 특정 작업(side effect(== 부수효과))을
   * 처리하기 위해 사용하는 Hook
   * 컴포넌트가 렌더링 되거나상태가 변경될 때 실행되는 작업을 정의할 수 있다.
   * 
   * useEffect(() => {
   *  // 실행한 작업(side effect)
   * 
   ...
   * // 의존성 매열은 이 useEffect가 언제 실행될지를 결정함
   * // -> 의존성 배열이 빈 배열일 경우, 이 useEffect는 컴포넌트가 처음 렌더링 될 때 딱 한번만 수행
   * 
   */

  // Restore 컴포넌트가 처음 렌더링 될 때 getWithdrawnMemberList, getDeleteBoardList 함수를 실해
  useEffect(() => {
    getWithdrawnMemberList();
    getDeleteBoardList();
  }, [])

  // withdreawnMembers 또는 deleteBoards 상태가 변경될 때마다 실행
  useEffect(() => {
