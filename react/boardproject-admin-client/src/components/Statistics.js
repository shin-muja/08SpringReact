...
export default function Statistics() {
  ...
  const getMaxLikeCount = async () => {
    try {
      ...
      console.log("가장 좋아요 수 게시글 조회 중 예외 발생: ", error);
    }
  }
  ...
  // readCountData, likeCountData, commentCountData에 변화가 감지될 때 콜백함수 내용
  ...
  return (
    <div>
      ...
      <section className="statistics-section">
        ...
        <p>게시글 조회 수 : {likeCountData.likeCount}</p>
      ...
      <section className="statistics-section">
        ...
        <p>게시글 조회 수 : {commentCountData.commentCount}</p>
...
const NewMembers = () => {
  ...
  return ( 
    <div className="new-members">
      <h2>신규 가입 회원 ({newMembers?.length})</h2>
      <h3>[ 7일 이내 가입 회원 ]</h3>
      <table border={1}>
        <thead>
          <tr>
