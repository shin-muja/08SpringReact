...
// Context는 React에서 컴포넌트 계층 구조(트리)를 통해 데이터를 효율적으로 전달하기 위한 매커니즘
...
export const AuthProvider = ({ children}) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8080/admin/login', // '요청주소'
        { // body 데이터
          memberEmail : email,
          memberPw: password
        },
        {"Content-Type" : "application/json"}// header 영역
      );
  ...
  const handleLogout = async () => {
    ...
    try{
      const response = await axios.get(
        'http://localhost:8080/admin/logout', // '요청주소'
        // body 데이터
        {"Content-Type" : "application/json"}// header 영역
      );
  ...
  return (
    // AuthContext.Provider : 데이터 제공하는 역할
